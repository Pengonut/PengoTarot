// PengoTarot/Enchantments/PlanetCeresEnchantment.cs
#nullable enable
using System.Linq;
...
{
    public sealed class PlanetCeresEnchantment : EnchantmentModel
    {
        ...
        public override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay? cardPlay)
        {
            ...
            {
                var ceresCards = player.PlayerCombatState!.AllCards
                    .Where(c => c.Enchantment is PlanetCeresEnchantment)
                    .ToList();

                foreach (var card in ceresCards)
                {
                    var clone = PlanetMarsPower.CreateCloneForPlayer(card, player);
                    CardCmd.PreviewCardPileAdd(
                        await CardPileCmd.AddGeneratedCardToCombat(clone, card.Pile?.Type ?? PileType.Discard, player));
                }
            }
        }
    }
