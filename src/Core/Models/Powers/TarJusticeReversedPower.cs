...
{
    /// <summary>
    /// 占卜-正义（逆位）效果 power：挂在玩家身上，让玩家的攻击牌获得「正义-逆」侵蚀（打出后消耗）。
    /// 战斗开始（AfterApplied）给当前所有攻击牌上侵蚀；后续新进入战斗的攻击牌（AfterCardEnteredCombat）也上。
    ...
    public sealed class TarJusticeReversedPower : PowerModel
    {
        ...
        /// <summary>本回合是否已触发过（每回合第一张攻击牌消耗；触发后本回合其余牌不再消耗且侵蚀标记消失）。</summary>
        ...
        {
            // 该玩家回合开始：重置「已触发」，重新给所有攻击牌上侵蚀（右上角图标恢复）
        ...
        public override async Task AfterCardPlayed(PlayerChoiceContext choiceContext, CardPlay cardPlay)
        {
            ...
            // 打出后消耗（每回合第一张攻击牌，回退为 AfterCardPlayed + CardCmd.Exhaust）
            await CardCmd.Exhaust(choiceContext, cardPlay.Card);
            // 本回合其余攻击牌的侵蚀标记消失（右上角图标消失，直到下回合）
            ClearOtherAfflictions(Owner, cardPlay.Card);
        }

        /// <summary>给玩家所有尚未侵蚀的攻击牌上正义-逆侵蚀标记（战斗开始/每回合开始）。</summary>
        private static async Task AfflictCards(Creature owner)
        {
            ...
            {
                if (card.Type == CardType.Attack && card.Affliction == null)
                    await CardCmd.Afflict<TarJusticeReversedAffliction>(card, 1m);
            }
        }

        /// <summary>清除本玩家其他攻击牌的正义-逆侵蚀标记（触发一次后本回合图标消失）。</summary>
        private static void ClearOtherAfflictions(Creature owner, CardModel played)
        {
            var allCards = owner.Player?.PlayerCombatState?.AllCards ?? Array.Empty<CardModel>();
            foreach (var card in allCards)
            {
                if (card != played && card.Affliction is TarJusticeReversedAffliction)
