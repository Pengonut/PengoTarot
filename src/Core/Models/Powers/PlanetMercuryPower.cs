// PengoTarot/Powers/PlanetMercuryPower.cs
#nullable enable
...
{
    public sealed class PlanetMercuryPower : PowerModel
    {
        ...
        public override async Task AfterSideTurnEnd(PlayerChoiceContext choiceContext, CombatSide side, IEnumerable<Creature> participants)
        {
            ...
            var drawPile = PileType.Draw.GetPile(PairedPlayer);
            var topCards = drawPile.Cards.Take(5).ToList();
            if (topCards.Count == 0) return;

            var prompt = new LocString("gameplay_ui", "PLANET_MERCURY_SELECTION_PROMPT");
            var prefs = new CardSelectorPrefs(prompt, 0, topCards.Count);

            var discarded = (await CardSelectCmd.FromSimpleGrid(
                choiceContext,
                topCards,
                base.Owner.Player!,
                prefs
            ...
            var combatState = PairedPlayer.Creature.CombatState;
